...
- 原理：不论屏幕多大，都将其看做xxx个rem，有：
  `根font-size:1rem=元素实际尺寸：元素rem尺寸`
  得到：
  `元素rem尺寸=1rem*元素实际尺寸/根font-size`
  同时也可由得到：
  `元素实际尺寸=根font-size*元素rem尺寸/1rem`
- rem布局的好处是非常便于适配不同屏幕尺寸,只要改变根f-s就能等比改变全局大小，实现总体控制
